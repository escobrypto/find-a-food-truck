      ...
      <div className="hero-split" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,alignItems:"center"}}>
        <div>
          ...
          <div className="au d4" style={{marginTop:48,paddingTop:28,borderTop:"1px solid rgba(255,255,255,.06)"}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,.2)",fontFamily:"var(--mono)",letterSpacing:".1em",marginBottom:14}}>NOW ACCEPTING FOUNDING VENDOR PARTNERS</div>
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
              {["BBQ","Mexican","Southern","Asian","Breakfast","Beverages"].map(c=><div key={c} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.05)",borderRadius:10,padding:"7px 14px"}}>
                <div style={{width:6,height:6,borderRadius:99,background:"var(--red)",animation:"pulse 3s infinite"}}/>
                <span style={{fontSize:12,color:"rgba(255,255,255,.35)",fontWeight:500,fontFamily:"var(--sans)"}}>{c}</span>
              </div>)}
            </div>
          </div>
        </div>
        ...
        <div className="hero-right au d3" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"1fr 1fr 1fr",gap:12,height:520}}>
          {[
            {name:"BBQ & Smoked",icon:"🔥",spots:"1 of 2 open",bg:"rgba(229,77,46,.06)",br:"24px 12px 12px 12px",span:"1/2",r:"1/3"},
            {name:"Mexican / Latin",icon:"🌮",spots:"2 of 2 open",bg:"rgba(199,125,21,.05)",br:"12px 24px 12px 12px",span:"2/3",r:"1/2"},
            {name:"Asian Fusion",icon:"🍜",spots:"2 of 2 open",bg:"rgba(124,58,237,.04)",br:"12px 12px 24px 12px",span:"2/3",r:"2/3"},
            {name:"Southern / Soul",icon:"🍗",spots:"1 of 2 open",bg:"rgba(45,140,60,.04)",br:"12px 12px 12px 24px",span:"1/3",r:"3/4"},
          ].map(({name,icon,spots,bg,br,span,r},i)=>
            <div key={i} className="lift" onClick={()=>go("/pricing")} style={{gridColumn:span,gridRow:r,background:bg,border:"1px solid rgba(255,255,255,.04)",borderRadius:br,padding:22,cursor:"pointer",display:"flex",flexDirection:"column",justifyContent:"space-between",position:"relative",overflow:"hidden"}}>
              <div><div style={{fontSize:36,marginBottom:8}}>{icon}</div><div style={{fontFamily:"var(--sans)",fontWeight:700,fontSize:14,color:"#fff"}}>{name}</div><div style={{fontFamily:"var(--sans)",fontSize:12,color:"rgba(255,255,255,.3)",marginTop:2}}>Founding Partner category</div></div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{color:"var(--red)",fontSize:11,fontWeight:700,fontFamily:"var(--mono)"}}>{spots}</span><span style={{color:"rgba(255,255,255,.2)",fontSize:10}}>Apply →</span></div>
            </div>
          )}
        </div>
      </div>
...
function Ticker(){
  const items=TRUCKS.length>0?TRUCKS.map(t=>`${t.img} ${t.name}`):["🚚 Founding Partner spots open","🔥 BBQ","🌮 Mexican","🍗 Southern","🍜 Asian","🧇 Breakfast","🥤 Beverages","📬 Apply now","⚡ Category-protected","🔒 Limited to 10"];
...
function TruckFinder({go}){
  ...
  return<section style={{minHeight:"100vh",background:"var(--bg)",padding:"120px 40px 80px"}}>
    <div style={{maxWidth:1240,margin:"0 auto"}}>
      <div className="au"><SH>Find a truck</SH><p style={{fontSize:16,color:"var(--sub)",fontFamily:"var(--body)",marginLeft:13,fontWeight:300}}>{TRUCKS.length>0?`${TRUCKS.filter(t=>t.status==="active").length} trucks serving RVA`:"Directory launching soon — vendors applying now"}</p></div>
  ...
  return<section style={{minHeight:"100vh",background:"var(--bg)",padding:"120px 40px 80px"}}>
    <div style={{maxWidth:1240,margin:"0 auto"}}>
      <div className="au" style={{marginBottom:44}}>
        ...
        <p style={{fontSize:16,color:"var(--sub)",fontFamily:"var(--body)",marginLeft:13,fontWeight:300}}>{EVENTS.length>0?`${EVENTS.length} upcoming · Verified vendors get early access & priority selection`:"Events launching soon · Verified vendors get early access & priority selection"}</p>
      </div>
      {EVENTS.length===0&&<div className="au d1" style={{textAlign:"center",padding:"80px 20px"}}>
        <div style={{fontSize:48,marginBottom:16,opacity:.3}}>📅</div>
        <h3 style={{fontSize:22,fontFamily:"var(--serif)",color:"var(--ink)",marginBottom:8}}>Events are coming</h3>
        <p style={{color:"var(--sub)",fontSize:15,fontFamily:"var(--body)",fontWeight:300,maxWidth:400,margin:"0 auto",lineHeight:1.7}}>We're lining up Richmond's best food truck events. Founding Vendor Partners get first access to every event posted.</p>
        <Btn style={{marginTop:24}} onClick={()=>go("/pricing")}>Become a Founding Partner →</Btn>
      </div>}
      {/* Featured */}
...
function MemberDash({go}){
  const[tab,setTab]=useState("profile");const truck=TRUCKS[0]||{name:"",cuisine:"",owner:"",phone:"",schedule:"",price:"",desc:"",menu:[],bookings:0,revenue:0,plan:"free",verified:false};
